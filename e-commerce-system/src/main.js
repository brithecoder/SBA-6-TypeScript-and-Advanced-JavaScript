...
async function main(itemListBody) {
    ...
    try {
        ...
        const productInstances = rawProductDataArray.map((productData) => {
            const instance = new product.Product(productData.id, productData.title, productData.description, productData.category, productData.price, productData.discountPercentage, productData.images[0]);
...
function renderItemList(productDataArray, itemListBody) {
    if (itemListBody) {
